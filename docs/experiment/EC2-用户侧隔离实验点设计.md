...
目标模型列表：[`assess-plan.json`](../../experiment/user-side/assess-plan.json) 的 **`families.<name>.models`**（Layer 2–3 探测）；[`sites.json`](../../experiment/user-side/sites.json) 的 **`supported_models`** 为文档摘录（Layer 1 对照，不驱动探测）。配置分工见 [`CONFIG.md`](../../experiment/user-side/CONFIG.md)。

**模型族（评估主轴，对齐用户 Key + model 心智）**

| 族 `family` | 主 Agent（应用场景） | Layer 2 wire | Layer 3 |
|-------------|----------------------|--------------|---------|
| **gpt** | Codex | `chat` · `responses` | OpenCode（探针）+ Codex |
| **anthropic** | Claude Code | `chat` · `messages` | OpenCode + Claude |
| **other** | — | `chat` | OpenCode only |

多族站点 Layer 2–3 须 **`--family`**；单族可省略。`sites.json` 的 **`protocol`** 仅限制 wire 与传输面的交集（不决定测哪些族）。

**传输 protocol**（`sites.json` 的 `protocol` 字段，实现提示）
...
Layer 3 按 **`--family`** 展开 Agent（`maas.py get assess_agents --site <id> --family gpt`）；Codex 经 LiteLLM **桥接**，Claude/OpenCode **直通**。

一键（1+2+3 单 Agent）：

```bash
./scripts/assess-source.sh --site <site-id> --family gpt --agent codex --smoke
```
