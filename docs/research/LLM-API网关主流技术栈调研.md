...
|----|------|
| **编写日期** | 2026-06-03 |
| **修订日期** | 2026-06-04（v3：总称改为 LLM API 网关；「中转站」收窄为商业 Token 站俗称；v3.1：补 §5.6 Sub2API 专节并校正技术栈表） |
...
| **Sub2API** | Go 1.25 · Gin · **Ent** | **Vue 3** · Vite · Tailwind | **PostgreSQL 15+** | Redis 7+ | `internal/gateway`；订阅账号池、拼车、内置支付（§5.6） |
...
| [Sub2API](https://github.com/Wei-Shaw/sub2api) | 同赛道独立实现 | 订阅配额分发、账号池与拼车运营；对外 `/v1/*` 与 Coding Agent 叙事见 **§5.6** | E1 |
...
### 5.6 Sub2API（订阅配额分发网关）

[Sub2API](https://github.com/Wei-Shaw/sub2api)（[Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)）是面向 **AI 产品订阅配额二次分发** 的自托管 **LLM API 网关**：平台聚合上游 **OAuth 订阅账号** 与 **API Key 账号池**，向终端用户签发平台 **API Key**，负责鉴权、调度、Token 级计费与请求转发。中文社区常将其与 New API 并列讨论（UI 与运营叙事相近），但 **代码库为独立实现**（Ent + Vue，默认 **PostgreSQL-only**），**不能** 用 New API 的 `relay/channel` 目录或数据库结构直接对照阅读。

#### 5.6.1 定位：与 New API、商业 Token 站的分工

| 维度 | Sub2API | New API | 商业 Token 站（`sites.json`） |
|------|---------|---------|-------------------------------|
| **主要目标** | 自建「卖 Key / 拼车」平台，管理订阅账号池 | 通用多上游聚合 + 丰富 RelayFormat | 购买现成平台 Token，零运维 |
| **上游形态** | 强调 **OAuth 订阅**（Claude Pro 等）+ API Key 混池 | Channel 接各云厂商 Key / 云凭证 | 不透明 |
| **运营闭环** | **内置支付**（支付宝、微信、Stripe、易支付等，E1） | 倍率计费为主；支付常外接 | 站方已集成 |
| **实现栈** | Go · Gin · **Ent** · Vue 3 · PG 15+ | Go · Gin · GORM · React/Semi · SQLite/MySQL/PG | 未知 |
| **许可证** | **LGPL-3.0**（E1） | **AGPL-3.0** | — |
| **本仓库实测** | **无** E3/E4 | 原型待补（`newapi-prototype`） | 按站点 reports |

**生态关系（E1–E2）**：官方 README 与赞助商列表中出现大量 **基于 Sub2API 运营的 relay 服务**（面向 Claude Code / Codex / Gemini CLI）。用户使用这些服务时，体验上接近 **商业 Token 站**；运维方则可能自托管 Sub2API 或使用官方 demo / 托管域（`sub2api.org`、`pincc.ai`，E1）。

#### 5.6.2 相对 New API 的能力差异（E1）

| 能力 | Sub2API（README / 文档） | New API（本仓库对照） |
|------|--------------------------|------------------------|
| 多账号与调度 | 智能选号、**粘性会话**（sticky）、每用户/每账号并发与限速 | Channel 权重、优先级、`KEY-CHANNEL_ID` |
| 计费 | Token 级用量与成本核算；`RUN_MODE=simple` 可隐藏 SaaS 计费（E1） | 分组倍率、`ratio_setting`、预扣费 |
| 支付 | **内置**，无需另部署支付服务（E1） | 通常外接或运营自定义 |
| 管理台 | Vue 3；支持 iframe 嵌入外部工单等（E1） | React / Semi 或 Bun 构建线 |
| 扩展账号类型 | **Antigravity** 专用路径 `/antigravity/v1/messages`、`/antigravity/v1beta/`（E1） | 以 Channel 类型 + `RelayFormat` 扩展 |
| 数据层 | **PostgreSQL 15+ 必选** | SQLite / MySQL / PostgreSQL |
| 默认端口 | **8080**（Setup Wizard，E1） | **3000** |

**协议面（E1，须部署后 probe）**：README 与社区材料宣称兼容 **Claude Code、Codex、Gemini CLI** 等；通用入站仍落在 OpenAI / Anthropic 兼容 **`/v1/*`** 族（与 §5.5、`assess-protocol` 四项探测一致）。**不得** 因「Sub2API 系 relay 品牌」推断 `/v1/responses` 已对 Codex 开放 — 须对 **实际 `base_url`** 跑 Layer 1–2。
...
│   ├── cmd/server/       # 入口
│   ├── internal/
│   │   ├── gateway/      # API 网关核心（转发、路由）
│   │   ├── handler/      # HTTP 处理
│   │   ├── service/      # 业务（账号池、计费、调度）
│   │   └── model/        # 领域模型
│   └── ent/              # Ent schema（数据层）
├── frontend/             # Vue 3 管理台与用户面
└── deploy/               # install.sh、docker-compose、docker-deploy.sh
```

与 New API 对照时：读 Sub2API 应抓住 **`internal/gateway` → handler → 上游账号选择**；读 New API 应抓住 **`controller.Relay` → `relay/channel` Adaptor**（见 [NewAPI 技术栈全景](./NewAPI技术栈全景.md)）。

#### 5.6.4 部署要点（E1）

| 方式 | 说明 |
|------|------|
| **脚本安装** | `deploy/install.sh` → systemd，依赖本机 PG 15+、Redis 7+ |
| **Docker Compose** | `deploy/docker-deploy.sh` 或 `docker-compose.local.yml`（推荐本地目录便于迁移） |
| **简易模式** | `RUN_MODE=simple` + 生产需 `SIMPLE_MODE_CONFIRM=true` — 隐藏 SaaS/计费，适合内网或小团队（E1） |
| **升级** | 管理台「检查更新」一键升级（E1） |

**反向代理（Codex / 多账号）**：Nginx 默认丢弃带下划线的 Header（如 `session_id`），会破坏多账号 **粘性路由**；须在 `http` 块设置 `underscores_in_headers on;`（E1）。

#### 5.6.5 Coding Agent 接入注意（E1）

| Agent / 场景 | 要点 | 证据 |
|--------------|------|------|
| **Claude Code** | 常规：`ANTHROPIC_BASE_URL` 指向网关根（含 `/v1` 或 Antigravity 专用基址，见 README） | E1 |
| **Claude Code + Antigravity** | 使用 `/antigravity` 基址；**勿** 在同一会话混用 Anthropic 原生渠道与 Antigravity 渠道（README 警告） | E1 |
| **Claude Code Plan Mode** | Antigravity 路径下 Plan Mode **可能无法自动退出**；变通：`Shift+Tab` 手动退出后再批准/拒绝计划（E1） |
| **Codex** | 除端点 probe 外，前置 Nginx 须处理 `underscores_in_headers`（§5.6.4） | E1 |
| **OpenCode** | 以 `POST /v1/chat/completions` 为主，与 §10 矩阵一致 | E0 + E1 |
| **本仓库 L3–L5** | **待补**：自托管 Sub2API 或某 Sub2API 系商业站需单独登记 `sites.json` 并写 reports | — |

#### 5.6.6 何时选 Sub2API vs New API

| 场景 | 更倾向 |
|------|--------|
| 要 **拼车 / 订阅账号池 / 内置支付** 一条龙建站 | **Sub2API** |
| 要 **MIT/AGPL 选型**、SQLite 单机、Responses/Realtime/Suno 等 **最全 RelayFormat** | **New API**（或 One API MIT 参考） |
| 仅 **个人用 Key 调模型**、不需运营台 | 商业 Token 站或 LiteLLM，未必需要 Sub2API |
| **三 Agent 生产 E2E** | 无论选谁，均须 **Layer 1–2 probe + `t_*`**；Sub2API **无** 本仓库现成 E3 结论 |

本仓库 **未** 将 Sub2API 纳入 `upstream/pull.sh`；若需对照实现，请自行 `git clone` 到 `upstream/sub2api/`（本地 gitignored，勿提交）。
...
| 订阅拼车 / 内置支付 / 账号池运营 | **Sub2API** 自托管 | OAuth 订阅 + API Key 混池、粘性调度；见 §5.6 | E1 |
...
- [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) · [Demo](https://demo.sub2api.org/) · [支付配置](https://github.com/Wei-Shaw/sub2api/blob/main/docs/PAYMENT.md)（§5.6）
...
**摘要**：LLM API 网关实现栈可归纳为 **Go/Gin 系（One API → New API；Sub2API 为同赛道独立实现，见 §5.6）** 与 **Python/FastAPI 系（LiteLLM）** 两条主线；商业 Token 站（俗称中转站）实现不透明，**协议面不可推测**。接入 Coding Agent 时，先以 **Layer 1–2（assess-platform + assess-protocol）** 完成源原生探测，再以 **`run-source-agent-test` / `t_*` 与 reports 完成 Layer 3 与 E2E L3–L5**；产品文档（E1）与站点实测（E3）须分开引用。
